...
This thesis tackles a case standard TAMP handles poorly: the robot often cannot see the object it must act on, because other objects block its view. The approach is to make that unseen space part of the planning problem. Each region the robot cannot see becomes a named, box-shaped cell---a \emph{Semantic Boxel}---placed around the detected objects and the volumes they hide; the planner reasons about which Boxel might hold the target, plans a sensing action to look inside, and replans as observations arrive. The remainder of this chapter explains why existing planners struggle with this and how Boxels address it.
