...
\subsection{Sensitivity to free-space resolution}
\label{subsec:resolution}

The free-space leaf size is auto-set per scene to the largest visible object's footprint plus a small margin (\cref{ch:methods}); on these scenes that \emph{auto\_cell} value is $\approx$9\,cm for the random-pairs goals (holding, find-and-tray-stack) and $\approx$6\,cm for stack. To test whether this heuristic sits at a tuned operating point or on a flat optimum, the \texttt{semantic} variant was swept across a $\sim$4$\times$ range of leaf sizes: a 5\,cm floor below \emph{auto\_cell} (\texttt{semantic+mbs0.05}, \cref{sec:baselines}) and two coarse points above it at 1.5$\times$ and 2$\times$ \emph{auto\_cell} (0.135 and 0.18\,m on random-pairs; 0.09 and 0.12\,m on stack).

\Cref{tab:resolution} reports success rate across the four resolution points. The axis is essentially flat. On stack the four arms are identical seed-for-seed (61.3\,\% at every leaf size, 184/300 solves). On holding success stays within seed noise (40--46\,\%). Only find-and-tray-stack shows a mild $\sim$4\,pp drop as the leaf coarsens. The ``coarser hurts'' cross-over expected of a resolution-sensitive method does not appear anywhere in the 1$\times$--2$\times$ range. Coarsening does change the representation, however: it shrinks the free-space partition by 30--40\,\% (median total Boxels: holding 35\,$\to$\,20, find-and-tray-stack 45\,$\to$\,27) and the grounded-fact count by a similar margin, while planning time edges down. Read together with the \texttt{mbs0.05} floor below \emph{auto\_cell}, the partition is insensitive to the leaf-size knob across the whole $\sim$4$\times$ range tested; \cref{ch:discussion} interprets why.
...
\begin{tabular}{lrrrr}
\toprule
goal & 0.05\,m & \emph{auto} & 1.5$\times$ & 2$\times$ \\
\midrule
holding             & 46.3\,\% & 42.3\,\% & 40.3\,\% & 45.7\,\% \\
find-and-tray-stack & 33.0\,\% & 39.8\,\% & 35.3\,\% & 35.3\,\% \\
stack               & 61.3\,\% & 61.3\,\% & 61.3\,\% & 61.3\,\% \\
\bottomrule
\end{tabular}
\caption{Success rate of the \texttt{semantic} variant across free-space leaf sizes, from a 5\,cm floor below \emph{auto\_cell} to 2$\times$ above it. Coarse-point sizes are per-scene (\emph{auto\_cell} $\approx$9\,cm on random-pairs, $\approx$6\,cm on stack). The axis is flat: stack identical seed-for-seed, holding within seed noise, find-and-tray-stack a mild coarsening cost.}
...
\Cref{fig:tampura} compares per-episode times on our \texttt{holding} task --- the closest analogue to TAMPURA's hidden-object \emph{Find Die} task (its \textit{Partial Observability} times are in Table~II of \cite{curtis2024partially}) --- with TAMPURA's reported numbers. Two cautions make this a framing exercise rather than a benchmark. First, what the spans measure: TAMPURA's 57.0\,$\pm$\,38\,s (Table~II) includes executing the selected controllers in simulation, so it is a per-episode figure, not planner time alone; the comparable quantity is therefore our per-episode \emph{wall-clock} (mean 13.7\,s, success-only, which likewise bundles PyBullet execution and replanning). Second, reliability runs the other way --- our \texttt{semantic} variant succeeds on 42\,\% of holding episodes, while TAMPURA's discounted return of 0.63\,$\pm$\,0.30 (Table~I, $\gamma=0.98$, binary terminal reward) implies a success rate of at least 63\,\%. We are no more expensive end-to-end but less reliable, on a marginally easier goal (\emph{Find Die} also requires returning home; our \texttt{holding} goal does not). The comparison is architectural --- a probabilistic learned-MDP planner versus our deterministic knowledge-literal planning with replanning --- run on different environments, so it benchmarks neither side; \cref{ch:discussion} unpacks the framing.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{tampura_wallclock_comparison.png}
    \caption{Per-episode wall-clock on \texttt{holding} (mean\,$\pm$\,std, success-only) against TAMPURA's Partial Observability time (mean\,$\pm$\,std, Table~II), which includes simulated controller execution and is likewise a per-episode figure. On reliability the order reverses: holding-\texttt{semantic} success is 42\,\% against TAMPURA's inferred $\geq$\,63\,\%.}
    \label{fig:tampura}
\end{figure}
