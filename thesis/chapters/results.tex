...
\Cref{fig:tampura} compares per-episode times on our \texttt{holding} task --- the closest analogue to TAMPURA's hidden-object \emph{Find Die} task (its \textit{Partial Observability} planning times are in Table~II of \cite{curtis2024partially}) --- with TAMPURA's reported numbers. Two cautions make this a framing exercise rather than a benchmark. First, TAMPURA's 57.0\,$\pm$\,38\,s (Table~II) includes executing the selected controllers in simulation, so it is a per-episode figure rather than planning time alone; the like-for-like quantity is our per-episode \emph{wall-clock} (mean 13.7\,s, success-only), which likewise bundles PyBullet execution and replanning. Second, reliability runs the other way --- our \texttt{semantic} variant succeeds on 42\,\% of holding episodes, while TAMPURA's discounted return of 0.63\,$\pm$\,0.30 (Table~I, $\gamma=0.98$, binary terminal reward) implies a success rate of at least 63\,\%. We are cheaper end-to-end but less reliable, on a marginally easier goal (\emph{Find Die} also requires returning home; our \texttt{holding} goal does not). The comparison is architectural --- a probabilistic learned-MDP planner (LAO*) versus our deterministic knowledge-literal planning with replanning --- and the two run on different environments, so it benchmarks neither system; \cref{ch:discussion} unpacks the framing.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{tampura_wallclock_comparison.png}
    \caption{Per-episode time on \texttt{holding}: our wall-clock (semantic variant; mean\,$\pm$\,std, success-only; right-skewed, median 8.5\,s) and TAMPURA's reported Partial Observability time (mean\,$\pm$\,std over 20 trials, Table~II). Both are per-episode times (TAMPURA's includes simulated controller execution); on reliability the order reverses, holding-\texttt{semantic} succeeding on 42\,\% of episodes against TAMPURA's inferred $\geq$\,63\,\%.}
    \label{fig:tampura}
\end{figure}
