...
On stack (\cref{fig:success-stack}), the difficulty axis is the stack height. All variants solve $h=2$ trivially (97\,\%); semantic and semantic+mbs0.05 degrade gracefully to 74\,\% at $h=3$ and 13\,\% at $h=4$, while uniform collapses from 97\,\% to 20\,\% to 1\,\%. The semantic and semantic+mbs0.05 curves are indistinguishable seed-for-seed. This decline with height is expected: each added layer is another pick-and-place that must succeed in sequence, and a cube that settles even slightly off its planned pose forces the loop to replan from the corrected state. Taller towers accumulate more such corrections, so on stack the adaptive variants fail predominantly by exhausting the replan budget (\texttt{replan\_limit}; \cref{fig:failure-modes}) rather than by any failure to find a plan; uniform collapses faster because its order-of-magnitude higher per-call planning cost (\cref{tab:headline}) leaves little headroom for those replans.
