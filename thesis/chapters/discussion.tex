...
are bundled into a single delta. Both planners are single-threaded. Our per-episode comparison
(\cref{subsec:tampura}) re-runs TAMPURA on the same machine as our system, so
that comparison is not confounded by hardware; only TAMPURA's \emph{published}
per-step time was measured on different hardware, a Xeon with a marginally
higher base clock (2.5 vs 2.0\,GHz) than ours, which if anything slightly
favours TAMPURA there.
