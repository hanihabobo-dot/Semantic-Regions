...
are bundled into a single delta. Both systems run a single-threaded classical search; the only multi-threaded
part of our pipeline is PyBullet's batched ray-casting for perception, which
speeds perception but not the planning search the comparison turns on. The per-episode figures we compare
