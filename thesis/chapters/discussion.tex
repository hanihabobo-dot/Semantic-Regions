...
--- the closest analogue to our \texttt{holding} task --- we are roughly an
order of magnitude cheaper end-to-end (13.7\,s vs 166\,s per episode) but
less reliable (42\,\% vs 55\,\% success), on a marginally easier goal.
...
Overall, the comparison's conclusion is that a deterministic knowledge-literal planner with replanning is sufficient to reach the goal on this class of partially observable tabletop tasks---cheaper to run and simpler to model than TAMPURA's learned probabilistic policy, though less reliable---and that it can plan over occlusions which a sub-symbolic visibility representation leaves implicit.
