...
\Cref{fig:tampura} sets our \texttt{holding} results against real TAMPURA,
re-run on the same hardware; \texttt{holding} --- find a hidden object
...
published 57\,$\pm$\,38\,s (the \textit{Partial Observability} column of
Table~II in \cite{curtis2024partially}) is a \emph{per-step} planning time
including simulated controller execution, not a per-episode figure; rather
than compare across mismatched units we re-ran the released \emph{Find Die}
environment on this thesis's machine, where it takes 166\,$\pm$\,85\,s per
successful episode against our 13.7\,s wall-clock (success-only) --- roughly
an order of magnitude apart, end-to-end, on the same CPU. The published
per-step number tells the same story: our matched per-step cost is 5.9\,s.
On reliability the order runs against us: our holding success rate is
42\,\%, below the local TAMPURA run's 55\,\% (itself consistent with the
$\geq$\,63\,\% implied by its 0.63\,$\pm$\,0.30 discounted return, Table~I).
We are cheaper end-to-end but less reliable, on a marginally easier goal
--- \emph{Find Die} also requires returning home, which ours does not.
