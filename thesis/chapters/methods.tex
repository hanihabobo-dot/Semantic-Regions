...
The system implemented in this thesis realises this model only approximately. Rather than solving the POD problem directly, it determinises the sensor model $\mathcal{O}$ optimistically, assuming each sensing action yields the outcome the plan expects, and recovers by reactive replanning (\cref{sec:kif_belief}). The model above therefore states the target semantics, including reaching the goal with certainty; the optimistic determinise-and-replan realisation trades that certainty for tractability, and the soundness limits it incurs are consolidated in \cref{sec:limitations}.
