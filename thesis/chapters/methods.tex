...
\begin{figure}[H]
    ...
    \includegraphics[width=0.65\textwidth]{Boxelization.png}
    \caption{Adaptive Semantic Discretization Process. (a) Initial scene with robot viewpoint and objects. (b) Objects are bounded by dedicated Boxels. (c) Occlusion regions are calculated and defined as new Boxels. (d) Remaining free space is recursively partitioned into Free Space Boxels.}
...
\begin{figure}[H]
    ...
    \caption{The same partition applied to a PyBullet scene. Object bounding cuboids enclose the visible cubes; red wireframe shadows mark the volumes occluded by each object. Free-space cells (not rendered) fill the remaining workspace.}
...
\begin{enumerate}
    ...
    \item \textbf{Occlusion-Aware Subdivision:} For each object resting on the support surface, the space it occludes from the fixed camera viewpoint is computed as a shadow Boxel; objects held by the gripper or otherwise off the surface do not cast shadows. A shadow that does not meet another region stays a single Boxel; only a shadow that would overlap another object or occluded region is split---by depth into near and far parts and around any intervening visible obstacles. As the robot acts, the set of shadow Boxels is updated: sensing a region resolves its shadow, and discovering a previously hidden object adds a shadow for that object. A relocated object, however, is not given a new shadow at its destination. Re-creating shadows for moved objects could admit plans that never terminate, so it is deliberately omitted; handling it safely would require additional belief bookkeeping and is left to future work. This explicitly represents regions the robot currently cannot observe, so the planner can target them with information-gathering actions.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Semantic partition: object boxels and free space where the scene is empty.}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Uniform-grid baseline: hundreds of cells of fixed size cover the workspace.}
...
The agent's knowledge is represented directly in the PDDL state. The implemented domain is untyped STRIPS---it declares no PDDL types, so object categories are ordinary predicates rather than type declarations---and the belief over object locations is carried by a single Know-If fluent.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{sim/replan_cycle.png}
    \caption{The reactive-replan trigger. The action log reads ``sense shadow\_of\_purple\_object --- target not here'': execution has rejected the optimistic add-effect, so the loop marks that shadow empty in the belief and replans, now searching only the remaining shadows.}
    \label{fig:replan-cycle}
\end{figure}
