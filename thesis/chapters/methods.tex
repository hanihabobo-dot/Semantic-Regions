...
\paragraph{Manipulation model.} The manipulation actions run on a deliberately simple model, summarised here because it shapes how the results should be read (\cref{sec:limitations} lists the full set of simplifications). Grasps are top-down at a single fixed height offset, and a picked object is rigidly attached to the gripper by a constraint, so it cannot slip or be dropped in transit. A \texttt{place} or \texttt{stack} lowers the object and lets the simulator settle it under gravity; a small hardcoded lift afterwards, invisible to the planner, leaves the arm at a collision-free start for the next move. A run counts as a success only if a physics check confirms the goal still holds after settling, so a plan that reaches the goal symbolically but leaves, for example, a toppled tower is recorded as a \emph{false success} and counted as a failure (\cref{subsec:failure-modes}), not a success. The success rates are therefore honest, but the stacking results in particular are bounded by this manipulation model as much as by the planner.
