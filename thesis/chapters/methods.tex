...
We build the Boxel abstraction through a process of \textit{adaptive semantic discretization}. This process dynamically generates a set of Semantic Boxels ($\mathcal{BX}$) by partitioning the workspace based on detected objects and the occlusions they create, and is re-run as the scene changes so the partition stays current. \Cref{fig:boxelization} illustrates the process, and \cref{fig:boxelization-real} shows the resulting partition on a simulated PyBullet scene.
\begin{figure}[H]
