...
More formally, a deterministic state-space model can be described as a tuple $M =\allowbreak \langle S, s_0, S_G,\allowbreak Act, A, f \rangle$, where:
\begin{itemize}
    ...
    \item $s_0 \in S$: The specific \textit{initial state} from which planning begins, e.g.\ $\{\texttt{(ontable A)},\allowbreak \texttt{(ontable B)},\allowbreak \texttt{(handempty)}\}$.
    ...
    \item $f(a, s) \rightarrow s'$: A deterministic \textit{state transition function}. Given a state $s$ and an applicable action $a \in A(s)$, $f(a,s)$ returns the unique successor state $s'$. For example, applying \textit{pick blockA} to $\{\texttt{(ontable A)},\allowbreak \texttt{(ontable B)},\allowbreak \texttt{(handempty)}\}$ yields $\{\texttt{(ontable B)},\allowbreak \texttt{(holding A)}\}$.
\end{itemize}
