...
\begin{itemize}
    \item \textbf{Procedural Component (Conditional Generator):} This is the actual code (e.g., a Python function) that, given some input values (which can be discrete symbols or other continuous values), produces a (potentially infinite) sequence of output values. For example, an \ac{ik} stream, given an object, its target pose, and a grasp, generates robot configurations that achieve that end-effector pose; because inverse kinematics is generally multi-valued (a redundant arm admits many joint configurations for the same end-effector pose), the generator yields these configurations one at a time on demand rather than a single answer. A collision-checking stream, given a trajectory, would test if it is collision-free.
