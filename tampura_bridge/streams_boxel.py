"""
PDDLStream Streams for Semantic Boxel TAMP — BRIDGE COPY (audit #120).

BRIDGE COPY: a fork of the shared streams.py used ONLY by the TAMPURA find_dice
bridge (tampura_bridge/).  It is robot-agnostic -- it threads a RobotModel
through IK / collision / motion so the streams run on TAMPURA's Panda (finger
joints [12,13], panda_grasptarget link 14).  The shared streams.py is left
untouched so the eval sweep, which imports it, is unaffected.  Keep this in
sync manually if the shared streams change.
...
from tampura_bridge.robot_utils_boxel import (
    ARM_JOINT_INDICES, END_EFFECTOR_LINK, FINGER_JOINTS,
    JOINT_LIMITS_LOW, JOINT_LIMITS_HIGH, JOINT_RANGES,
    REST_POSES, RenderingLock, RobotModel, _DEFAULT_PANDA,
...
class BoxelStreams:
    ...
    def __init__(self, registry: BoxelRegistry, robot_id: int = None,
        ...
        # Robot-specific joint/link model (audit #120).  Defaults to OUR Panda
        # so existing callers are unaffected; the find_dice bridge passes a
        # model built for TAMPURA's Panda.
        self.robot = robot if robot is not None else _DEFAULT_PANDA
        ...
        self.home_config = RobotConfig(
            joint_positions=np.array(self.robot.rest_poses),
            name="q_home"
        )
    ...
    def _pybullet_ik(self, ee_pos: np.ndarray,
        ...
        """
        if seed is None:
            seed = list(self.robot.rest_poses)
        saved_joints = None
        ...
        with RenderingLock(pc):
            try:
                ...
                for i, angle in zip(self.robot.arm_joints, seed):
                ...
                joint_positions = p.calculateInverseKinematics(
                    ...
                    endEffectorLinkIndex=self.robot.ee_link,
                    targetPosition=ee_pos.tolist(),
                    targetOrientation=ee_orn.tolist(),
                    lowerLimits=self.robot.joint_low.tolist(),
                    upperLimits=self.robot.joint_high.tolist(),
                ...
                if np.any(arm_joints < self.robot.joint_low - 0.1) or \
                   np.any(arm_joints > self.robot.joint_high + 0.1):
                    return None
                ...
                if saved_joints is not None:
                    for i, angle in zip(self.robot.arm_joints, saved_joints):
                        p.resetJointState(self.robot_id, i, angle,
                                          physicsClientId=pc)

    def _ik_seeds(self):
        """Yield IK seed configurations: REST_POSES first, then perturbations."""
        rest = np.array(self.robot.rest_poses)
        for offset in self._IK_SEED_OFFSETS[:self.IK_NUM_SEEDS]:
            seed = rest + np.array(offset)
            seed = np.clip(seed, self.robot.joint_low, self.robot.joint_high)
            yield seed.tolist()
    ...
    def _random_config(self) -> np.ndarray:
        ...
        return np.random.uniform(self.robot.joint_low, self.robot.joint_high)
